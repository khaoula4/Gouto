...
public class modif extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dateEdt.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                DatePickerDialog datePickerDialog = new DatePickerDialog(modif.this,
        ...
        String pos = intent.getStringExtra("pos_neg");
        String prof = intent.getStringExtra("profendeur");
        String pro = intent.getStringExtra("province");
        String com = intent.getStringExtra("commune");
        String deb = intent.getStringExtra("debit");
        String ex = intent.getStringExtra("exp_rec");
        String date = intent.getStringExtra("date");
        key = intent.getStringExtra("key");

        X.getEditText().setText(x);
        Y.getEditText().setText(y);
        Code.getEditText().setText(code2);
        t.getEditText().setText(pos);
        Profendeur.getEditText().setText(prof);
        Province.getEditText().setText(pro);
        Commune.getEditText().setText(com);
        Debit.getEditText().setText(deb);
        e_r.getEditText().setText(ex);
    ...
    public void modif2(View view) {

        if(!validatecom() | !validateprov() |  !validateProf() | !validatedate() | !validateCode() |  !validateX() | !validateY() | !validatex() | !validatepos()){
            return;
        }
        else{
            String x,y,pro,prof,Com,date,code,Deb,ex,pos;
            ex = e_r.getEditText().getText().toString();
            pos = t.getEditText().getText().toString();

            x =X.getEditText().getText().toString();
            y = Y.getEditText().getText().toString();
            pro = Province.getEditText().getText().toString();
            prof = Profendeur.getEditText().getText().toString();
            Com = Commune.getEditText().getText().toString();
            date = dateEdt.getText().toString();
            code =Code.getEditText().getText().toString();
            Deb =Debit.getEditText().getText().toString();

            //     Forage forage = dataSnapshot.getValue(Forage.class);



            myRef.child(key).child("x").setValue(x);
            myRef.child(key).child("y").setValue(y);
            myRef.child(key).child("code").setValue(code);
            myRef.child(key).child("exp_rec").setValue(ex);
            myRef.child(key).child("pos_neg").setValue(pos);
            myRef.child(key).child("province").setValue(pro);
            myRef.child(key).child("commune").setValue(Com);
            myRef.child(key).child("debit").setValue(Deb);
            myRef.child(key).child("profendeur").setValue(prof);
            myRef.child(key).child("date_Realisation").setValue(date);
            ...
            Intent intent = new Intent(modif.this,Home.class);

            startActivity(intent);
            finish();
            Toast.makeText(this,"Forage Modifié", Toast.LENGTH_LONG).show();
        }
